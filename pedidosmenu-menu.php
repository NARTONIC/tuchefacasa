			...
			<h3 class="text-center">
                            <?php
                                if(isset($_POST['confirm'])){
                                    ...
                                    ?>
                            <form action="pedidosmenu-menu.php" method="post">
                                <?php
                                    list($menus) = Menu::getMenu();
                                    
                                    foreach ( $menus as $menu ) {
                                        echo "<input type='radio' name='menu' onchange='showPlatos(this.value)' value='".$menu->getValueEncoded("id_menu")."'>".$menu->getValueEncoded("nombre")." - ".$menu->getValueEncoded("precio")."€";
                                        echo "<br />";
                                    }
                                ?>
                                    <div id="txtHint"><h3 class="text-center">Seleccione un menú para ver sus platos</h3></div>
                                </h3>
                            </form>
            <?php
                }
                function bebidas(){
                    $_SESSION['id_menu'] = $_POST['menu'];
                    header('Location: pedidosmenu-bebidas.php');
                }
    ?>
	</div>
</div>
